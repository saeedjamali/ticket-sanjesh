...
export default function PreviousYearStudentStats({ filters = {} }) {
  ...
  useEffect(() => {
    fetchPreviousYearStats();
  }, [filters]);

  const fetchPreviousYearStats = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("token");

      const params = new URLSearchParams({
        yearFilter: "previous", // برای دریافت آمار سال گذشته
      ...
      const response = await fetch(
        `/api/students/previous-year-stats?${params}`,
        {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.message || "خطا در دریافت آمار سال گذشته");
      }
    } catch (error) {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    await fetchPreviousYearStats();
    setRefreshing(false);
    toast.success("آمار سال گذشته بروزرسانی شد");
  };
      ...
      <div className="bg-white p-4 rounded-lg shadow-md mb-6">
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-800">
            آمار دانش‌آموزان ثبت نام شده سال گذشته
          </h3>
          <button
    ...
    <div className="bg-white p-6 rounded-lg shadow-md mb-6 ring-2 ring-gray-200">
      <div className="flex items-center justify-between my-4">
        <h3 className="text-lg font-semibold text-gray-800 flex items-center">
          <FaChartBar className="ml-2 text-purple-600" />
          آمار دانش‌آموزان ثبت نام شده سال گذشته
        </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-green-50 p-4 rounded-lg border border-green-200">
              <div className="flex items-center justify-between">
                ...
                <FaMale className="text-green-500 text-2xl" />
              </div>
            </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-orange-600 text-sm font-medium">
                    درصد ثبت نام در سامانه
                  </p>
                  <p className="text-2xl font-bold text-orange-800">
                    {stats.examCenterStats?.registrationPercentage || 0}%
                  </p>
                  <p className="text-xs text-orange-600">
                    از{" "}
                    {stats.examCenterStats?.totalStudents?.toLocaleString(
                      "fa-IR"
                    ) || 0}{" "}
                    نفر
                  </p>
                </div>
                <FaSchool className="text-orange-500 text-2xl" />
              </div>
            </div>
          </div>
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="text-left">
                        ...
                        {district.examCenterTotal > 0 ? (
                          <>
                            <p className="text-xs text-green-600 font-medium mt-1">
                              {district.registrationPercentage || 0}% ثبت نام در
                              سامانه
                            </p>
                            <p className="text-xs text-gray-500">
                              از{" "}
                              {district.examCenterTotal?.toLocaleString(
                                "fa-IR"
                              ) || 0}{" "}
                              نفر
                            </p>
                          ...
                          <p className="text-xs text-red-600 mt-1">
                            آمار ExamCenterStats موجود نیست
                          </p>
                        )}
                      </div>
                    </div>
                            ...
                            >
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <div className="text-left">
                                  ...
                                  {course.examCenterTotal > 0 && (
                                    <div className="text-xs text-green-600 mt-1">
                                      {course.courseRegistrationPercentage || 0}
                                      % ثبت نام
                                    </div>
                                  )}
                                </div>
                                  ...
                                  {course.examCenterTotal > 0 && (
                                    <>
                                      ...
                                      <span className="text-gray-500">
                                        از {course.examCenterTotal} نفر آمار
                                      </span>
                                    </>
                                  )}
                                </div>
                              </div>
                              {/* نمودار درصد ثبت نام دوره */}
                              {course.examCenterTotal > 0 &&
                                course.courseRegistrationPercentage > 0 && (
                                  <div className="mt-2">
                                    <div className="bg-gray-200 rounded-full h-1.5">
                                      <div
                                        ...
                                        style={{
                                          width: `${Math.min(
                                            course.courseRegistrationPercentage,
                                            100
                                          )}%`,
                                        }}
                                      ></div>
                      ...
                      <div className="mt-3 pt-3 border-t">
                        <div className="flex justify-between items-center text-xs mb-1">
                          ...
                          <span className="font-medium text-green-700">
                            {district.registrationPercentage || 0}%
                          </span>
                        </div>
