...
const OrganizationalUnitTypeManager = () => {
  ...
  const fetchItems = async () => {
    try {
      ...
      const response = await fetch(
        `/api/organizational-unit-types?page=${pagination.currentPage}&limit=10${searchParam}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingItem
        ? `/api/organizational-unit-types/${editingItem._id}`
        : "/api/organizational-unit-types";
  ...
  const handleEdit = (item) => {
    ...
    setFormData({
      ...
      unitTypeTitle: item.unitTypeTitle,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`/api/organizational-unit-types/${id}`, {
        method: "DELETE",
      });
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingItem ? "ویرایش نوع واحد" : "افزودن نوع واحد جدید"}
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, unitTypeCode: e.target.value })
                  }
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.unitTypeTitle}
                  onChange={(e) =>
                    setFormData({ ...formData, unitTypeTitle: e.target.value })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  required
                />
...
export default OrganizationalUnitTypeManager;
