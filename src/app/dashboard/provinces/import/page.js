...
export default function ImportProvincesPage() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch("/api/provinces/import", {
        method: "POST",
        body: formData,
        credentials: "include",
      });
  ...
  const downloadTemplate = () => {
    window.location.href = "/api/provinces/template";
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800">
          افزودن گروهی استان‌ها
        </h1>
        <Link
          href="/dashboard/provinces"
          className="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300"
        >
          بازگشت به مدیریت استان‌ها
        </Link>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="mb-6">
          <h2 className="text-lg font-semibold mb-2">
            راهنمای افزودن گروهی استان‌ها
          </h2>
          <p className="text-gray-600">
            برای افزودن گروهی استان‌ها، لطفا مراحل زیر را دنبال کنید:
          </p>
          <ol className="list-decimal list-inside mt-2 space-y-2 text-gray-600 pr-4">
            <li>فایل نمونه اکسل را دانلود کنید</li>
            <li>اطلاعات استان‌ها را در فایل وارد کنید</li>
