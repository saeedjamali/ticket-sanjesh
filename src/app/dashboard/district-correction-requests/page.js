...
export default function DistrictCorrectionRequestsPage() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await fetch(
        "/api/admin/profile-correction-requests/district"
      );
  ...
  useEffect(() => {
    if (user && user.role === "districtTransferExpert") {
      fetchRequests();
    }
  }, [user]);
            ...
            <div className="flex items-center gap-3 text-white">
              ...
              <div>
                <h1 className="text-2xl font-bold">
                  مدیریت درخواست‌های اصلاح مشخصات منطقه
                </h1>
                <p className="text-blue-100 text-sm">
                  بررسی و پاسخ به درخواست‌های اصلاح مشخصات کاربران
                </p>
              </div>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-lg p-12 text-center">
            ...
            <p className="text-gray-600">
              {requests.length === 0
                ? "هیچ درخواست اصلاح مشخصاتی برای منطقه شما ثبت نشده است."
